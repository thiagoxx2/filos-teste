...
print(f"Total unique CSS lines recovered: {len(recovered_lines)}")
...
if missing:
    ...
    # Write reconstructed CSS to file
    css_content = "\n".join([recovered_lines[i] for i in sorted(recovered_lines.keys())])
    with open("/Users/shayenefreita/FACULDADE FILOS/css/responsive.css", "w", encoding="utf-8") as out:
        out.write(css_content)
    print("Successfully wrote reconstructed css/responsive.css!")
